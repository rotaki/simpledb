...
namespace smartdb {
  TEST(query, scantest2) {
    ...
    layout lt1(sch1);
    auto us1 = std::static_pointer_cast<update_scan>(std::make_shared<table_scan>(tx.get(), "T1", lt1));
    ...
    layout lt2(sch2);
    auto us2 = std::static_pointer_cast<update_scan>(std::make_shared<table_scan>(tx.get(), "T2", lt2));
    us2->before_first();
    ...
    auto s1 = std::static_pointer_cast<scan>(std::make_shared<table_scan>(tx.get(), "T1", lt1));
    auto s2 = std::static_pointer_cast<scan>(std::make_shared<table_scan>(tx.get(), "T2", lt2));
    auto s3 = std::static_pointer_cast<scan>(std::make_shared<product_scan>(s1, s2));

    // selecting all records where A=C
    term t(expression("A"), expression("C"));
    predicate pred(t);
    std::cout << "The predicate is " << pred.to_string() << std::endl;
    auto s4 = std::static_pointer_cast<scan>(std::make_shared<select_scan>(s3, pred));

    // projecting on [B,D]
    std::vector<std::string> c = {"B", "D"};
    auto s5 = std::static_pointer_cast<scan>(std::make_shared<project_scan>(s4, c));
    while (s5->next()) {
