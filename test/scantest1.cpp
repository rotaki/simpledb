...
TEST(query, scantest1_test) {
  ...
  layout lt(sch1);
  auto s1 = std::static_pointer_cast<update_scan>(
      std::make_shared<table_scan>(tx.get(), "T", lt));
  ...
  auto s3 =
      std::static_pointer_cast<scan>(std::make_shared<select_scan>(s2, pred));
  std::vector<std::string> fields = {"B"};
  auto s4 = std::static_pointer_cast<scan>(
      std::make_shared<project_scan>(s3, fields));
  while (s4->next()) {
