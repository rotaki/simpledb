...
int stat_info::blocks_accessed() { return mNumBlocks; }

int stat_info::records_output() { return mNumRecs; }

int stat_info::distinct_values(const std::string &pFldName) {
  return 1 + (mNumRecs / 3); // this is wildly inaccurate
}

stat_manager::stat_manager(table_manager *pTM, transaction *pTx) : mTM(pTM) {
  refresh_statistics(pTx);
}

stat_info stat_manager::get_stat_info(const std::string &pTblName,
...
stat_info stat_manager::calc_table_stats(const std::string &pTblName,
                                         const layout &pLt, transaction *pTx) {
  int numRecs = 0;
  int numBlocks = 0;
