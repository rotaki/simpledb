...
void view_manager::create_view(const std::string &pVName,
                               const std::string &pVDef, transaction *pTx) {
...
std::string view_manager::get_view_def(const std::string &pVName,
                                       transaction *pTx) {
  std::string result;
  layout lt = mTM->get_layout("viewcat", pTx);
  table_scan tS(pTx, "viewcat", lt);
