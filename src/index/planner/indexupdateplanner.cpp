...
namespace simpledb {
  ...
  int index_update_planner::execute_insert(insert_data* pID, transaction* pTx) {
    ...
    std::shared_ptr<plan> p = std::static_pointer_cast<plan>(std::make_shared<table_plan>(pTx, tblName, mMM));
  ...
  int index_update_planner::execute_delete(delete_data* pDD, transaction* pTx) {
    ...
    std::shared_ptr<plan> p = std::static_pointer_cast<plan>(std::make_shared<table_plan>(pTx, tblName, mMM));
    p = std::static_pointer_cast<plan>(std::make_shared<select_plan>(p, pDD->pred()));
    std::map<std::string, index_info> indexes = mMM->get_index_info(tblName, pTx);
    std::shared_ptr<update_scan> s = std::dynamic_pointer_cast<update_scan>(p->open());
    if (!s) {
      throw std::runtime_error("type conversion failed");
  ...
  int index_update_planner::execute_modify(modify_data* pMD, transaction* pTx) {
    ...
    std::shared_ptr<plan> p = std::static_pointer_cast<plan>(std::make_shared<table_plan>(pTx, tblName, mMM));
    p = std::static_pointer_cast<plan>(std::make_shared<select_plan>(p, pMD->pred()));
  ...
  int index_update_planner::execute_create_table(create_table_data* pCTD, transaction* pTx) {
    mMM->create_table(pCTD->table_name(), pCTD->new_schema(), pTx);
    return 0;
  }

  int index_update_planner::execute_create_view(create_view_data* pCVD, transaction* pTx) {
    mMM->create_view(pCVD->view_name(), pCVD->view_def(), pTx);
    return 0;
  }

  int index_update_planner::execute_create_index(create_index_data* pCID, transaction* pTx) {
