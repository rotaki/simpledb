...
block_id bt_page::split(const int &pSplitPos, const int &pFlag) {
...
block_id bt_page::append_new(const int &pFlag) {
...
void bt_page::format(const block_id &pBlk, const int &pFlag) {
...
void bt_page::make_default_record(const block_id &pBlk, const int &pPos) {
...
void bt_page::transfer_recs(const int &pSlot, bt_page *pDest) {
  int destSlot = 0;
