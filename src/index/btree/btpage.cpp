...
namespace simpledb {
bt_page::bt_page(std::shared_ptr<transaction> pTx,
                 std::shared_ptr<block_id> pCurrentBlk,
...
std::shared_ptr<block_id> bt_page::split(const int &pSplitPos,
                                         const int &pFlag) {
...
std::shared_ptr<block_id> bt_page::append_new(const int &pFlag) {
  std::shared_ptr<block_id> blk = std::make_shared<block_id>(
      mTx->append(mCurrentBlk->file_name())); // todo fix
  mTx->pin(*blk);
  format(blk, pFlag);
  return blk;
}
...
void bt_page::make_default_record(std::shared_ptr<block_id> pBlk,
                                  const int &pPos) {
...
int bt_page::get_child_num(const int &pSlot) { return get_int(pSlot, "block"); }

void bt_page::insert_dir(const int &pSlot, const constant &pVal,
                         const int &pBlkNum) {
  insert(pSlot);
...
void bt_page::insert_leaf(const int &pSlot, const constant &pVal,
                          const rid &pRid) {
  insert(pSlot);
  set_val(pSlot, "dataval", pVal);
...
void bt_page::set_int(const int &pSlot, const std::string &pFldName,
                      const int &pVal) {
  int pos = fld_pos(pSlot, pFldName);
  mTx->set_int(*mCurrentBlk, pos, pVal, true);
}

void bt_page::set_string(const int &pSlot, const std::string &pFldName,
                         const std::string &pVal) {
  int pos = fld_pos(pSlot, pFldName);
  mTx->set_string(*mCurrentBlk, pos, pVal, true);
}

void bt_page::set_val(const int &pSlot, const std::string &pFldName,
                      const constant &pVal) {
  int type = mLt->get_schema().type(pFldName);
  if (type == schema::integer) {
...
void bt_page::copy_record(const int &pFrom, const int &pTo) {
  std::shared_ptr<schema> sch =
      std::make_shared<schema>(mLt->get_schema()); // todo fix
