...
term::term(const term &pT) : mLhs(pT.mLhs), mRhs(pT.mRhs) {}
...
int term::reduction_factor(plan *pPlan) const {
  ...
  if (mLhs.is_field_name() && mRhs.is_field_name()) {
    ...
    return std::max(pPlan->distinct_values(lhsName),
                    pPlan->distinct_values(rhsName));
  }
...
constant term::equates_with_constant(const std::string &pFldName) const {
  if (mLhs.is_field_name() && mLhs.as_field_name() == pFldName &&
      !mRhs.is_field_name()) {
    return mRhs.as_constant();
  } else if (mRhs.is_field_name() && mRhs.as_field_name() == pFldName &&
             !mLhs.is_field_name()) {
