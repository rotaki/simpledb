...
namespace simpledb {
project_scan::project_scan(const std::shared_ptr<scan> &pS,
                           const std::vector<std::string> &pFieldList)
    : mS(pS), mFieldList(pFieldList) {}

void project_scan::before_first() { mS->before_first(); }

bool project_scan::next() { return mS->next(); }
...
bool project_scan::has_field(const std::string &pFldName) {
  return std::find(mFieldList.begin(), mFieldList.end(), pFldName) !=
         mFieldList.end();
}

void project_scan::close() { mS->close(); }
} // namespace simpledb
