...
expression::expression(const constant &pVal) : mVal(pVal) {}

expression::expression(const std::string &pFldName) : mFldName(pFldName) {}
...
bool expression::is_field_name() const { return !mFldName.empty(); }

constant expression::as_constant() const { return mVal; }

std::string expression::as_field_name() const { return mFldName; }
