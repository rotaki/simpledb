...
namespace simpledb {
record_page::record_page(transaction *pTx, const block_id &pBlockId,
                         const layout &pLayout)
    : mTx(pTx), mBlockId(pBlockId), mLayout(pLayout) {
  mTx->pin(mBlockId);
}

int record_page::get_int(int pSlot, const std::string &pFldName) {
  int fldPos = offset(pSlot) + mLayout.offset(pFldName);
  return mTx->get_int(mBlockId, fldPos);
}

std::string record_page::get_string(int pSlot, const std::string &pFldName) {
  int fldPos = offset(pSlot) + mLayout.offset(pFldName);
  return mTx->get_string(mBlockId, fldPos);
}

void record_page::set_int(int pSlot, const std::string &pFldName, int pVal) {
  int fldPos = offset(pSlot) + mLayout.offset(pFldName);
  return mTx->set_int(mBlockId, fldPos, pVal, true);
}

void record_page::set_string(int pSlot, const std::string &pFldName,
                             const std::string &pVal) {
  int fldPos = offset(pSlot) + mLayout.offset(pFldName);
  return mTx->set_string(mBlockId, fldPos, pVal, true);
}

void record_page::remove(int pSlot) { set_flag(pSlot, empty); }
...
int record_page::next_after(int pSlot) { return search_after(pSlot, used); }

int record_page::insert_after(int pSlot) {
...
block_id record_page::block() const { return mBlockId; }

void record_page::set_flag(int pSlot, int pFlag) {
  mTx->set_int(mBlockId, offset(pSlot), pFlag, true);
}

int record_page::search_after(int pSlot, int pFlag) {
  int slot = pSlot;
  slot++;
  while (is_valid_slot(slot)) {
...
bool record_page::is_valid_slot(int pSlot) const {
  return offset(pSlot + 1) <= mTx->block_size();
}

int record_page::offset(int pSlot) const {
