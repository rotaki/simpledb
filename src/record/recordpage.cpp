...
std::string record_page::get_string(const int &pSlot,
                                    const std::string &pFldName) {
  int fldPos = offset(pSlot) + mLayout.offset(pFldName);
  return mTx->get_string(mBlockId, fldPos);
}

void record_page::set_int(const int &pSlot, const std::string &pFldName,
                          const int &pVal) {
  int fldPos = offset(pSlot) + mLayout.offset(pFldName);
  return mTx->set_int(mBlockId, fldPos, pVal, true);
}

void record_page::set_string(const int &pSlot, const std::string &pFldName,
                             const std::string &pVal) {
  int fldPos = offset(pSlot) + mLayout.offset(pFldName);
  return mTx->set_string(mBlockId, fldPos, pVal, true);
}

void record_page::remove(const int &pSlot) { set_flag(pSlot, empty); }
