...
namespace simpledb {
log_manager::log_manager(file_manager *pFileManager,
                         const std::string &pLogFile)
    : mFileManager(pFileManager), mLogFile(pLogFile) {
  int logSize = mFileManager->length(mLogFile);
  auto byteVec =
      std::make_shared<std::vector<char>>(mFileManager->block_size(), 0);
...
int log_manager::append(const std::vector<char> &pLogRecord) {
  std::unique_lock<std::mutex> lock(
      mMutex); // only one thread should add record to page
...
log_manager::log_iterator::log_iterator(file_manager *ppFileManager,
                                        const block_id &ppBlockId)
    : mmFileManager(ppFileManager), mmBlockId(ppBlockId) {

  auto byteVec =
      std::make_shared<std::vector<char>>(mmFileManager->block_size(), 0);
  mmPage = std::make_unique<page>(byteVec);
  move_to_block(mmBlockId);
}

bool log_manager::log_iterator::has_next() {
  return ((mmCurrentPos < mmFileManager->block_size()) ||
          (mmBlockId.number() > 0));
}
