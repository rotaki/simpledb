...
namespace simpledb {
  ...
  simpledb::simpledb(const std::string &pDirName, const int &pBlockSize, const int &pBuffSize) {
  ...
  simpledb::simpledb(const std::string &pDirName): simpledb(pDirName, mBlockSize, mBufferSize) {
  ...
  metadata_manager* simpledb::md_mgr() {
    return mMM.get();
  }
  
  file_manager* simpledb::file_mgr() {
    return mFM.get();
  }

  planner* simpledb::plnr() {
    return mP.get();
  }

  log_manager* simpledb::log_mgr() {
    return mLM.get();
  }

  buffer_manager* simpledb::buffer_mgr() {
    return mBM.get();
  }
}
