...
namespace simpledb {
basic_query_planner::basic_query_planner(metadata_manager *pMM) : mMM(pMM) {}

std::shared_ptr<plan> basic_query_planner::create_plan(query_data *pData,
                                                       transaction *pTx) {
  ...
  for (int i = 1; i < static_cast<int>(plans.size()); i++) {
    p = std::static_pointer_cast<plan>(
        std::make_shared<product_plan>(p, plans[i]));
  }
  ...
  return std::static_pointer_cast<plan>(
      std::make_shared<project_plan>(p, pData->fields()));
}
} // namespace simpledb
