...
std::shared_ptr<plan> planner::create_query_plan(const std::string &pCMD,
                                                 transaction *pTx) {
  parser p(pCMD);
...
int planner::execute_update(const std::string &pCMD, transaction *pTx) {
  ...
  if (obj->op() == object::insert) {
    ...
    return mUP->execute_create_table(
        static_cast<create_table_data *>(obj.get()), pTx);
  } else if (obj->op() == object::createview) {
    return mUP->execute_create_view(static_cast<create_view_data *>(obj.get()),
                                    pTx);
  } else if (obj->op() == object::createindex) {
    return mUP->execute_create_index(
        static_cast<create_index_data *>(obj.get()), pTx);
  } else {
    return 0;
  }
}
