...
namespace simpledb {
select_plan::select_plan(const std::shared_ptr<plan> &pPlan,
...
std::shared_ptr<scan> select_plan::open() {
  ...
  return std::static_pointer_cast<scan>(
      std::make_shared<select_scan>(s, mPred));
}

int select_plan::blocks_accessed() { return mPlan->blocks_accessed(); }
...
int select_plan::distinct_values(const std::string &pFldName) {
  if (!mPred.equates_with_constant(pFldName).is_null()) {
    ...
    if (!fldName.empty()) {
      return std::min(mPlan->distinct_values(pFldName),
                      mPlan->distinct_values(fldName));
    } else {
...
schema select_plan::get_schema() { return mPlan->get_schema(); }
} // namespace simpledb
